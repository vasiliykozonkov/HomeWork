public class Main {
    ...
    public static void main(String[] args) {
        ...
        Notifier notifier2 = new SmsNotifierDecorator(new EmailNotifier(email), phone);
        notifier2.send(message);
        System.out.println();

        System.out.println("📋 ПРИМЕР 3: Email + SMS + Telegram");
        Notifier notifier3 = new TelegramNotifierDecorator(
                new SmsNotifierDecorator(new EmailNotifier(email), phone), 
                telegram
        );
        ...
        Notifier notifier4 = new SmsNotifierDecorator(
                new EncryptionDecorator(new EmailNotifier(email)), 
                phone
        );
        ...
        Notifier notifier5 = new TelegramNotifierDecorator(
                new EncryptionDecorator(
                        new LoggingDecorator(new EmailNotifier(email))
                ), 
                telegram
        );
